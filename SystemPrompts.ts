export enum SystemPrompts {
  ...
  ImprovePrompt = `Please improve this prompt`,

  TalkToGPT = `You are friendly AI assistant Start with a friendly greeting and confirm the information provided by the user, including any inferred details relevant to their query.`,
...
description: A comprehensive analysis of the frame, highlighting interactions, objects like cars and their colors and brand, emotions, incidents, and any noticeable anomalies within the scene.Any movements like entry or exit of vehicles.
entitiesDetected: An array of major objects identified in the scene, described in string format.
The goal is to ensure a thorough understanding of the video content, frame by frame, providing users with an interactive and immersive experience.The result should contain nothing besides the returned JSON array.Example Output Format: [  {    "frameNumber": 1,    "description": "A boy looking at a girl, both standing in a park with a bat and ball visible on the ground. The boy appears curious, while the girl seems amused.",    "entitiesDetected": ["bat", "ball", "boy", "girl"]
  }
  // Additional frame objects follow
]
`;

export const DecideHowToProcessPrompt = `
...
export const VideoAudioSummarizationPrompt = `
...
export const VideoAIMainChatbotPrompt = `
...
export const DeliverTimeStampsPromptAudio = `
You're an AI assistant trained in video content analysis. Your task is to assist users by interpreting video data, including frame details and audio transcripts, and presenting this analysis in an engaging and accessible way. Here's how to proceed:
...
export const DeliverTimeStampsPromptVideo = `
You're an AI assistant trained in video content analysis. Your task is to assist users by interpreting video data, including frame details and audio transcripts, and presenting this analysis in an engaging and accessible way. Here's how to proceed:
